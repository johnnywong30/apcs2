// Team Yawrricks- Johnny Wong, Richard Wong, Zane Wang
// APCS2 pd1
 ...
 *  From the start position, check all possible moves in the four cardinal directions.
 *  Follow on this path until the hero reaches the exit.
 *  If the hero cannot reach the exit and reaches a dead end, backtrack
 *  until the hero reaches the exit.
...
class MazeSolver
{
  ...
  public void solve( int x, int y ) {
    ...
    //primary base case if maze is solved
    if ( solved ) {
	    System.exit(0);
    }
    //other base case(s) if hero is at exit
    else if ( maze[x][y] == EXIT) {
	    solved = true;
         System.out.println(this); // refresh
         return;
    }
    // if hero is at a wall
    else if ( maze[x][y] == WALL)   {
	    return;
    }
    // if hero is at a visited path
    else if ( maze[x][y] == VISITED_PATH) {
	    return;
    }
    // otherwise look for path to exit
    //recursive reduction
    else {
         maze[x][y] = VISITED_PATH;
         solve(x,y + 1); // up one
         solve(x,y - 1); // down one
         solve(x + 1,y); // right one
         solve(x - 1,y); // left one
